...
- Prompt 真源：`src/prompt-config/promptConfig.ts`，当前配置版本为 `badigao-v1.3.0`。
- 当前主视觉类型为 A/B/C/替换模式；替换模式包含“服装+场景替换、产品替换、服装替换”三种子模式。
...
| A/B/C 与替换模式 | 已可用 | A=品牌情绪，B=真实使用，C=功能卖点；替换模式百分百复刻参考图姿势、动作、图片视角和构图，不继承参考图人物与光影调性。 |
| 三种替换子模式 | 已可用 | 服装+场景替换、产品替换、服装替换；替换模式复用 A 类场景。 |
...
│  ├─ prompt-config/
│  │  └─ promptConfig.ts             # A/B/C、替换模式、场景、卖点、镜头、色调、Prompt 编译真源
│  ├─ data/
...
- C 类：没有场景列表，直接选择功能卖点
- 替换模式：复用 A 类五个场景；三种子模式为服装+场景替换、产品替换、服装替换
- 替换参考图角色：`replacement_reference`；姿势、动作、图片视角、景别、裁切、参考画幅、人物位置占比、留白与构图必须百分百复刻；人物身份、面孔、身体特征、服装设计和光影调性不得继承
- 含服装替换的子模式会把人物图动态标记为 `character_garment_reference`：除人物一致性外，可参考服装穿着位置、搭配、廓形、覆盖范围、褶皱与姿势贴合；产品设计细节仍以 `product_master` 为最高优先级
...
13. **替换模式画幅跟随首张替换参考图**：构图要求百分百复刻，因此正式 Gemini 生图会读取首张 `replacement_reference` 的像素比例并映射为最接近的受支持画幅；页面通用画幅不会覆盖参考构图。
14. **现有历史结果不会被新规则重算**：切换场景或替换模式后，旧任务仍可在历史中看到；只有重新生成的批次才使用新规则。

## 12. 修改任务时从哪里开始

| 需求类型 | 优先查看 |
| --- | --- |
| 改 A/B/C、替换模式、场景、卖点、镜头、色调或 Prompt | `src/prompt-config/promptConfig.ts` |
...
3. A/B/C/替换模式切换时场景、替换子模式、色调和卖点显示正确。
...
7. 对三种替换子模式分别调用 Prompt Preview：服装+场景替换应包含所选 A 类场景；产品替换和服装替换不得注入新场景；三者均应百分百锁定姿势、动作、图片视角与构图，明确排除参考图人物和光影调性，并保留产品失真负面词。
8. `GET /api/gemini/status` 返回预期配置状态。
9. 使用低成本参数生成 1 张图，确认实际场景、`generated/` 落盘和历史新增。
10. 打开结果、下载、设为参考图、恢复历史参数。
11. 局部涂抹编辑并确认新版本写入任务。
12. 刷新页面，确认项目、任务和上传图片恢复正常。
