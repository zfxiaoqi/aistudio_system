...
> 面向接手本项目的开发 Agent。本文基于 2026-07-16 的代码现状整理；当前分支为 `main`，基线提交为 `1d174be`（`feat: enhance image generation and editing workflow`）。
...
| 实际生图 Prompt 预览 | 已可用 | 前端防抖调用 Gemini Prompt Preview 接口。 |
...
最重要的业务原则：**产品还原优先于创意发挥**。风格参考图可影响视觉风格、光影、色彩、影调、景深、留白、姿势、动作、裁切、机位、背景、道具、物体位置与构图，但禁止复制参考图的主体、人物身份、脸、身体和服装设计。

Prompt 选项现状：

- A 类场景：海边自在、居家松弛、米色自在、蓝色洁净
...
2. 服务端重新编译 Prompt，并优先采用前端传入的已优化 Prompt。
3. 参考图按产品主图、产品细节、人物身份、风格参考排序并附加角色约束。
4. 逐张调用 Gemini；若命中 `IMAGE_SAFETY`，会用文本模型安全润色后重试一次。
5. 返回图片写入 `generated/`。
6. 前端创建 `Task`，保存参数快照、结果 URL、编辑版本和视频字段。
...
| `POST /api/gemini/prompt-preview` | 构造实际提交给 Gemini 的最终 Prompt 预览 | 当前前端 |
| `POST /api/gemini/generate-images` | Gemini 多参考图批量生图 | 当前前端主链路 |
...
5. `GET /api/gemini/status` 返回预期配置状态。
6. 使用低成本参数生成 1 张图，确认 `generated/` 落盘和历史新增。
7. 打开结果、下载、设为参考图、恢复历史参数。
8. 局部涂抹编辑并确认新版本写入任务。
9. 刷新页面，确认项目、任务和上传图片恢复正常。
...
- Vite 成功构建 1690 个模块。
- 本轮检查时 `127.0.0.1:3000` 未运行，因此没有把 API 在线响应作为本次通过项。
- 仓库没有测试脚本，未执行自动化业务测试。
